...
function FileWorkspace({ tool }: { tool: ToolDefinition }) {
  ...
  // 0 means "to the end of the file"; trimAudioFile treats endSeconds 0 as the
  // full duration, so an undetectable length exports the whole file instead of
  // silently truncating to a fixed default.
  ...
  // When the trimmer's source file changes, clear any duration/range from the
  // previous file synchronously during render (React's "adjust state on change"
  // pattern) so a new file never inherits a stale End time before its metadata
  // loads. A file the browser can't decode natively then keeps End at 0 = whole file.
  const [trimFileKey, setTrimFileKey] = useState<File | null>(null);
  const currentTrimFile = files[0] ?? null;
  if (isAudioTrim && currentTrimFile !== trimFileKey) {
    setTrimFileKey(currentTrimFile);
    setSourceDuration(0);
    setTrimStart(0);
    setTrimEnd(0);
  }
  ...
  const outputLabel = useMemo(() => {
    ...
    if (isVideo) return "High-quality video (WebM / MP4)";
  ...
  useEffect(() => {
    ...
    // Browsers that can't natively decode the format (e.g. Ogg Vorbis or WebM on
    // Safari/iOS) fire 'error' instead of 'loadedmetadata'. We keep End at 0 so
    // the whole file exports through the ffmpeg fallback rather than truncating,
    // and surface a note that the length is unknown.
